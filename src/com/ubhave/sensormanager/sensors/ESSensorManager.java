...
public class ESSensorManager
{
	...
	// 1. push sensors: sms, phone state sensors that receive notification about
	// events
	// 2. pull sensors: like mic sensor, accelerometer sensor, where sensor data
	// should be pulled from them. there are two sub-categories in this type,
	// (a) sensors with fixed window size, like accelerometer, mic sensors where
	// the sampling window is fixed (b) sensors with variable window size, like
